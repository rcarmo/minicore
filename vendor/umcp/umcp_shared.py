...
class MCPHTTPResponse:
    ...
    # Minicore: auxiliary async streams use connection-close framing.
...
def validate_http_response(response: Any, *, max_bytes: int) -> MCPHTTPResponse | None:
    ...
    if not isinstance(response.body, bytes):
        return None
    if response.stream is not None and (response.body or not hasattr(response.stream, "__aiter__")):
        return None
