...
test("redacts before persistence and strips terminal control", () => {
...
test("stable IDs, repeat lines, incarnation change and bounded entries", async () => {
  const stamp = new Date().toISOString();
...
test("collection child deadline and output cap are enforced", async () => {
...
test("partial long or old lines cannot exceed window/message bounds", () => {
  const stamp = new Date().toISOString();
